...
def piggy_points(score):
    ...
    opp_squared = score **2
    k = 3
    digit = opp_squared % 10
    while opp_squared >=1:
        new_digit = opp_squared % 10
        if new_digit < digit:
            digit = new_digit
        opp_squared = opp_squared //10
    return digit + k
...
def more_boar(player_score, opponent_score):
    ...
    if opponent_score < 10: 
        first_opp = 0
        second_opp = opponent_score 

    else: 
        while opponent_score >= 10: 
            second_opp  = opponent_score % 10
            opponent_score = opponent_score // 10
        first_opp = opponent_score


    if first_player < first_opp and second_player < second_opp: 
        return True
    else: 
        return False
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            current_score_gain = score0 - last_score
            current_score = score0
            current_high = running_high
            if current_score_gain > current_high:
                current_high = current_score_gain
                print("Player 0 has reached a new maximum point gain.", current_high, "point(s)!")
                
        elif who == 1:
            current_score_gain = score1 - last_score
            current_score = score1
            current_high = running_high
            if current_score_gain > current_high:
                current_high = current_score_gain
                print("Player 1 has reached a new maximum point gain.", current_high, "point(s)!")
        return announce_highest(who, last_score = current_score, running_high = current_high)
    return say
...
def make_averaged(original_function, trials_count=1000):
    ...
    def call_original(*args):
        ...
        while num_trials < trials_count:
            average_value_cal = average_value_cal + original_function(*args)
            num_trials += 1

        return average_value_cal / trials_count
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    no_roll = score + piggy_points(opponent_score)
    if more_boar(no_roll, opponent_score)
    # END PROBLEM 11
